...
async function fetchEventFromAPI(id) {
  try {
    const apiEventResponse = await fetch(
      `https://64b517e8f3dbab5a95c6afd3.mockapi.io/events/${id}`
    );
    const eventObject = await apiEventResponse.json();

    const apiOrgResponse = await fetch(
      `https://64b517e8f3dbab5a95c6afd3.mockapi.io/organizers/${eventObject.organizerId}`
    );
